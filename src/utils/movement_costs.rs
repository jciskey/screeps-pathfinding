...
/// let costs = screeps_pathfinding::utils::movement_costs::get_movement_cost_lcm_from_terrain(&room_terrain, plain_cost, swamp_cost);
///
/// let roomxy_costs_fn = screeps_pathfinding::utils::movement_costs::astar_movement_costs_from_lcm(&costs);
///
/// let xy = RoomXY::checked_new(24, 18).unwrap();
/// assert_eq!(roomxy_costs_fn(xy), Some(plain_cost as u32));
///
/// let position_costs_fn = screeps_pathfinding::utils::movement_costs::astar_movement_costs_from_lcm(&costs);
///
/// let pos = Position::new(
///     RoomCoordinate::try_from(24).unwrap(),
///     RoomCoordinate::try_from(18).unwrap(),
///     "E5N6".parse().unwrap(),
/// );
/// assert_eq!(position_costs_fn(pos), Some(plain_cost as u32));
/// ```
pub fn astar_movement_costs_from_lcm<T>(lcm: &LocalCostMatrix) -> impl Fn(T) -> Option<u32> + '_
where
    T: Into<RoomXY>,
{
    |xy| {
